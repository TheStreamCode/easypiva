...
import { ATECO_CATEGORIES, INPS_RATES } from "@/lib/constants"
...
export default function TargetNet() {
  ...
  const aliquotaImposta = nuovaAttivita ? 0.05 : 0.15

  const nettoAnnuo = nettoMensile * 12

  // Formula inversa:
  // Netto = Ricavi - Tasse - INPS
  // Tasse = (Ricavi * Coeff - INPS) * Aliquota
  // INPS = Ricavi * Coeff * AliquotaINPS (semplificato per GS)
  
  // Semplificazione:
  // Se GS:
  // INPS = Ricavi * Coeff * 0.2607
  // Tasse = (Ricavi * Coeff - INPS) * Aliquota = (Ricavi * Coeff - Ricavi * Coeff * 0.2607) * Aliquota
  // Netto = Ricavi - Tasse - INPS
  // Ricavi = Netto / (1 - Coeff * AliquotaINPS - Coeff * (1 - AliquotaINPS) * Aliquota)

  let ricaviNecessari = 0
  let inpsStimato = 0
  let tasseStimate = 0
  ...
  if (tipoInps === "gestioneSeparata") {
    ...
    ricaviNecessari = nettoAnnuo / denom
    inpsStimato = ricaviNecessari * coefficiente * aliquotaInps
    tasseStimate = (ricaviNecessari * coefficiente - inpsStimato) * aliquotaImposta
  } else {
    // Artigiani/Commercianti (approssimazione iterativa per gestire il minimale)
    const rates = INPS_RATES[tipoInps]
    const riduzione = riduzioneInps ? 0.65 : 1
    
    // Iterazione semplice per trovare i ricavi
    let low = nettoAnnuo
    ...
    for (let i = 0; i < 50; i++) {
      mid = (low + high) / 2
      const redditoLordo = mid * coefficiente
      
      let inps = rates.minimalContribution * riduzione
      if (redditoLordo > rates.minimalIncome) {
        inps += (redditoLordo - rates.minimalIncome) * rates.rateOverMinimal * riduzione
      }
      
      const tasse = Math.max(0, redditoLordo - inps) * aliquotaImposta
      const nettoCalcolato = mid - tasse - inps
      
      if (nettoCalcolato < nettoAnnuo) {
        low = mid
      } else {
        high = mid
      }
    ...
    inpsStimato = rates.minimalContribution * riduzione
    if (redditoLordo > rates.minimalIncome) {
      inpsStimato += (redditoLordo - rates.minimalIncome) * rates.rateOverMinimal * riduzione
    }
    tasseStimate = Math.max(0, redditoLordo - inpsStimato) * aliquotaImposta
  }
    ...
    <div className="max-w-4xl mx-auto flex flex-col gap-12 pb-16 pt-8">
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
          Quanto Fatturare per Netto
        </h1>
        <p className="text-lg text-zinc-600 dark:text-zinc-400">
          Scopri quanto devi fatturare per ottenere il netto mensile desiderato.
        </p>
      </div>
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="space-y-3">
              ...
              />
              <p className="text-sm text-zinc-500">Netto annuo: <span className="font-medium text-zinc-700 dark:text-zinc-300">{formatCurrency(nettoAnnuo)}</span></p>
            </div>
          ...
          <div className="flex flex-col gap-6 p-6 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 ring-1 ring-zinc-200 dark:ring-zinc-800">
            ...
            <div className="text-center py-8">
              ...
              <p className="text-5xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
                {formatCurrency(ricaviNecessari)}
              </p>
            ...
            <div className="space-y-4 text-sm pt-6 border-t border-zinc-200 dark:border-zinc-800">
              ...
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                ...
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(nettoAnnuo)}</span>
              </div>
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                <span>Tasse ({aliquotaImposta * 100}%)</span>
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(tasseStimate)}</span>
              </div>
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                <span>INPS</span>
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(inpsStimato)}</span>
              </div>
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                <span>Costi Forfettari</span>
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(ricaviNecessari - (ricaviNecessari * coefficiente))}</span>
              </div>
            </div>
            
            <p className="text-xs text-zinc-500 mt-6 leading-relaxed">
              * I calcoli sono approssimati e non tengono conto di eventuali contributi pregressi o deduzioni extra.
            </p>
          </div>
        </motion.div>
      </motion.div>
