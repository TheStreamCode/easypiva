...
export default function Planning() {
  ...
  const chartData = mesi.map((mese, index) => {
    cumulato += ricaviMensili[index]
    return {
      name: mese,
      Ricavi: ricaviMensili[index],
      Cumulato: cumulato,
    }
  })
    ...
    <div className="max-w-5xl mx-auto flex flex-col gap-12 pb-16 pt-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-lg text-zinc-600 dark:text-zinc-400">
          Inserisci i ricavi previsti mese per mese per monitorare il limite degli 85.000€.
        </p>
      </div>
      ...
      >
        ...
        {/* Proiezione e Grafico (Main Content) */}
            ...
            <div className={`p-8 rounded-2xl border transition-colors duration-300 ${
              ...
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <p className={`text-5xl md:text-6xl font-semibold tracking-tight ${
                  ...
                  {formatCurrency(totaleAnnuo)}
                </p>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2">
                  Limite Forfettario: {formatCurrency(LIMITS.ricavi)}
                </p>
              </div>

              {limiteSuperato && !uscitaImmediata && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-8 p-4 bg-amber-100/50 text-amber-900 dark:bg-amber-900/30 dark:text-amber-200 rounded-xl border border-amber-200 dark:border-amber-800/50 text-sm leading-relaxed"
                >
                  <strong className="font-semibold">Attenzione:</strong> Hai superato la soglia degli 85.000€. L'anno prossimo uscirai dal regime forfettario e passerai al regime ordinario, ma per l'anno in corso mantieni i benefici fiscali.
                </motion.div>
              )}
              ...
              {uscitaImmediata && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-8 p-4 bg-red-100/50 text-red-900 dark:bg-red-900/30 dark:text-red-200 rounded-xl border border-red-200 dark:border-red-800/50 text-sm leading-relaxed"
                >
                  <strong className="font-semibold">CRITICO:</strong> Hai superato la soglia dei 100.000€. Esci IMMEDIATAMENTE dal regime forfettario nell'anno in corso. Dovrai applicare l'IVA sulle fatture successive all'incasso che ha causato il superamento.
                </motion.div>
              )}
            </div>
          </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                  ...
                  <Line 
                    type="monotone" 
                    dataKey="Cumulato" 
                    name="Fatturato Cumulato"
                    stroke="var(--color-chart-1)" 
                    strokeWidth={3} 
                    dot={{ r: 4, fill: 'var(--color-chart-1)', strokeWidth: 2, stroke: 'var(--color-background)' }} 
