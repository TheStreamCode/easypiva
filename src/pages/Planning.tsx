          ...
          <div className="flex flex-col gap-4">
            {mesi.map((mese, index) => (
              <div key={mese} className="flex items-center gap-4">
                <Label
                  htmlFor={`mese-${index}`}
                  className="w-12 text-zinc-600 dark:text-zinc-400 font-medium"
                >
                  {mese}
                </Label>
                <Input
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <p
                  className={`text-5xl md:text-6xl font-semibold tracking-tight ${result.uscitaImmediata ? 'text-red-600 dark:text-red-400' : result.limiteSuperato ? 'text-amber-600 dark:text-amber-400' : 'text-zinc-900 dark:text-zinc-50'}`}
                >
                  {formatCurrency(result.totaleAnnuo)}
                </p>
              ...
              {result.warnings.map((warning) => (
                <motion.div
                  ...
                  className={`mt-8 p-4 rounded-xl border text-sm leading-relaxed ${warning.severity === 'critical' ? 'bg-red-100/50 text-red-900 dark:bg-red-900/30 dark:text-red-200 border-red-200 dark:border-red-800/50' : 'bg-amber-100/50 text-amber-900 dark:bg-amber-900/30 dark:text-amber-200 border-amber-200 dark:border-amber-800/50'}`}
                >
                  <strong className="font-semibold">
                    {warningCopy[warning.code as keyof typeof warningCopy]?.title ?? 'Attenzione'}:
                  </strong>{' '}
                  {warningCopy[warning.code as keyof typeof warningCopy]?.message ?? warning.code}
                </motion.div>
              ))}
            </div>
          </div>
            ...
            <div className="h-[400px] w-full min-w-0 pt-4">
              ...
              >
                ...
                >
                  ...
                  <ReferenceLine
                    y={LIMITS.ricavi}
                    label={{ position: 'top', value: 'Limite 85k', fill: '#f59e0b', fontSize: 12 }}
                    stroke="#f59e0b"
                    strokeDasharray="3 3"
                    opacity={0.5}
                  />
                  <ReferenceLine
                    y={LIMITS.uscitaImmediata}
                    label={{ position: 'top', value: 'Uscita 100k', fill: '#ef4444', fontSize: 12 }}
                    stroke="#ef4444"
                    strokeDasharray="3 3"
                    opacity={0.5}
                  />
