  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col gap-12 pb-16 pt-8">
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">Contributi INPS 2026</h1>
        <p className="text-lg text-zinc-600 dark:text-zinc-400">Simula i contributi previdenziali in base alla tua cassa.</p>
      </div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
        <motion.div variants={itemVariants} className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="space-y-3">
              ...
              <Input id="redditoLordo" type="number" value={redditoLordo} onChange={(e) => setRedditoLordo(Number(e.target.value))} className="text-lg" />
            </div>
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-8">
          <div className="flex flex-col gap-6 p-6 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 ring-1 ring-zinc-200 dark:ring-zinc-800">
            <h3 className="text-lg font-medium text-zinc-900 dark:text-zinc-100 border-b border-zinc-200 dark:border-zinc-800 pb-2">Riepilogo Contributi</h3>

            <div className="flex flex-col gap-4 text-sm">
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                <span>Contributo Fisso (Minimale)</span>
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.fisso)}</span>
              </div>
              <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                <span>Contributo Variabile (Eccedenza)</span>
                <span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.variabile)}</span>
              </div>
              ...
              <div className="flex justify-between items-center text-lg">
                ...
                <span className="font-semibold text-zinc-900 dark:text-zinc-100">{formatCurrency(result.totale)}</span>
              </div>
            </div>
            ...
            {(tipoInps === "artigiani" || tipoInps === "commercianti") && (
              <div className="pt-6 border-t border-zinc-200 dark:border-zinc-800">
                ...
                <ul className="space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
                  <li className="flex justify-between items-center"><span>16 Maggio 2026</span><span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.fisso / 4)}</span></li>
                  <li className="flex justify-between items-center"><span>20 Agosto 2026</span><span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.fisso / 4)}</span></li>
                  <li className="flex justify-between items-center"><span>16 Novembre 2026</span><span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.fisso / 4)}</span></li>
                  <li className="flex justify-between items-center"><span>16 Febbraio 2027</span><span className="font-medium text-zinc-900 dark:text-zinc-100">{formatCurrency(result.fisso / 4)}</span></li>
                </ul>
                <p className="text-xs text-zinc-500 mt-6 leading-relaxed">* I contributi variabili si pagano con il saldo e gli acconti delle imposte (Giugno/Novembre).</p>
              </div>
            )}

            {tipoInps === "gestioneSeparata" && (
              <div className="pt-6 border-t border-zinc-200 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">La Gestione Separata non ha rate fisse. Si paga in percentuale sul reddito effettivo tramite saldo e acconti (Giugno/Novembre).</p>
              </div>
            )}
          </div>
        </motion.div>
